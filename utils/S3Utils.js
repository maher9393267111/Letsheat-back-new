const {
  ...
  const singleFileDelete = async (fileId) => {
    ...
    const deleteParams = {
      Bucket: bucketName,
      Key: `letsheat/${fileId}`, // Assuming 'job/' prefix based on original code
    };
  ...
  const multiFilesDelete = async (images) => {
    ...
    for (const image of images) {
      ...
      const deleteParams = {
        Bucket: bucketName,
        Key: `letsheat/${fileId}`, // Assuming 'job/' prefix
      };
  
      console.log(`Queueing deletion for file: broher/${fileId}`);
      deletePromises.push(
        s3Client.send(new DeleteObjectCommand(deleteParams))
          ...
          .catch(error => {
            console.error(`Error deleting file broher/${fileId}:`, error);
